...
AS (SELECT fc.CaseId,
    ...
    WHERE fc.CaseFirstPaidDate IS NOT NULL
          AND fc.CaseCreatedDate >= DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)
...
WHERE cteCycleTimeSubmittedToPaid.CaseManagerName IS NOT NULL; -- AND cteCycleTimeSubmittedToPaid.Channel ='ADVISOR';
